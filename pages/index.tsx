...
const Home: NextPage = () => {
  ...
  useEffect(() => {
    ...
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${cities[numberRandom]}&appid=${process.env.WEATHER_KEY}`)
  ...
  const handleSearch = (event: any) => {
    if (event.key === 'Enter') {
      fetch(`https://api.openweathermap.org/data/2.5/weather?q=${search}&appid=${process.env.WEATHER_KEY}`)
        .then(response => response.json())
        .then(json => {
          if (json.cod === '404' || json.cod === '400'){
            setError({error: true, message: json.message}) 
